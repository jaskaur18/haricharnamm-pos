    ...
    <YStack gap="$3">
      ...
      <SurfaceCard gap="$2.5">
        ...
        {sale.returns.length === 0 ? (
          ...
          sale.returns.map((r) => (
            <YStack key={r._id} bg="$color3" rounded="$4" p="$2.5" gap="$1.5">
              <XStack justify="space-between" items="center">
                <YStack gap="$0.5">
                  <Paragraph fontWeight="700">{r.returnCode}</Paragraph>
                  <Paragraph color="$color7" fontSize="$1">{formatDateTime(r.createdAt)} · {paymentMethodLabel(r.refundMethod)}</Paragraph>
                </YStack>
                <Paragraph fontWeight="700">{formatCurrency(r.subtotal)}</Paragraph>
              </XStack>
              {r.items.map((item, i) => (
      ...
      <ResponsiveDialog open={returnOpen} onOpenChange={setReturnOpen} title="Create return" description="Select quantities to return.">
        <YStack gap="$3" py="$2">
          {sale.items.map((item) => (
            <XStack key={item._id} bg="$color3" rounded="$4" p="$2.5" justify="space-between" items="center" gap="$3">
              <YStack flex={1} gap="$0.5">
                <Paragraph fontWeight="700" fontSize="$2">{item.productName}</Paragraph>
                <Paragraph color="$color7" fontSize="$1">{item.variantLabel} · {formatNumber(item.remainingQty)} remaining</Paragraph>
              </YStack>
              <Input
          ...
          <FormField label="Refund note">
            <TextArea value={refundNote} onChangeText={setRefundNote} placeholder="Optional" style={{ minHeight: 60 }} bg="$color1" borderColor="$borderColor" />
          </FormField>
        ...
        <XStack gap="$2" flexWrap="wrap">
          <YStack flex={1} style={{ minWidth: 160 }}>
            <FormField label="Search">
              <Input value={search} onChangeText={setSearch} placeholder="Code, customer…" bg="$color1" borderColor="$borderColor" />
            </FormField>
          </YStack>
          <YStack style={{ minWidth: 120 }}>
            <FormField label="From">
              <Input value={fromDate} onChangeText={setFromDate} placeholder="YYYY-MM-DD" bg="$color1" borderColor="$borderColor" />
            </FormField>
          </YStack>
          <YStack style={{ minWidth: 120 }}>
            <FormField label="To">
              <Input value={toDate} onChangeText={setToDate} placeholder="YYYY-MM-DD" bg="$color1" borderColor="$borderColor" />
            </FormField>
          </YStack>
        ...
        <YStack flex={1} gap="$1.5" style={{ minWidth: 300 }}>
          {status === 'LoadingFirstPage' ? (
            ...
            sales.map((sale) => {
              ...
              return (
                <Button key={sale._id} unstyled onPress={() => setSelectedSaleId(sale._id)}>
                  <XStack
                    bg={isSelected ? '$color3' : '$color2'}
                    borderWidth={1}
                  ...
                  >
                    {isSelected ? (
                      <YStack bg="$color8" style={{ position: 'absolute' as any, left: 0, top: 8, bottom: 8, width: 3, borderRadius: 999 }} />
                    ) : null}
                    <YStack flex={1} gap="$0.5">
                      <Paragraph fontWeight="700" fontSize="$3">{sale.saleCode}</Paragraph>
                      <Paragraph color="$color7" fontSize="$1">
                        {formatDateTime(sale.createdAt)} · {paymentMethodLabel(sale.paymentMethod)}
                      </Paragraph>
                      <Paragraph color="$color7" fontSize="$1">{sale.customerName || 'Walk-in'}</Paragraph>
                    </YStack>
                    <YStack items="flex-end" gap="$0.5">
                      <StatusBadge status={sale.status} />
                      <Paragraph fontWeight="700" fontSize="$3">{formatCurrency(sale.total)}</Paragraph>
                      <Paragraph color="$color7" fontSize="$1">{formatNumber(sale.totalQty)} units</Paragraph>
                    </YStack>
                  </XStack>
                </Button>
              )
